...
func resolveTurn(bs *BattleState) []string {
	...
	if playerMove == "pass" && aiMove == "pass" {
		logEntries = append(logEntries, "Both passed. Nothing happened!")
...
func SwitchPokemon(bs *BattleState, newIdx int) error {
	...
	// Only allow switching after a knockout or at the start of a round
	currentCard := bs.GetActivePlayerCard()
	if currentCard != nil && currentCard.HP > 0 {
		// Can only switch if it's the winner's choice (after winning previous round)
		// For now, we'll allow switching between rounds
		return fmt.Errorf("can only switch after your Pokemon is knocked out")
	}
