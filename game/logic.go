...
func ProcessWebMove(state *GameState, move string, moveIdx *int) (map[string]interface{}, error) {
	...
	if state.WhoseTurn == "player" {
		...
		for {
			aiMove, aiMoveIdx := getAIMove(state.PendingPlayerMove, aiCard, state, state.AIActiveIdx)
			if aiMove == "surrender" {
				state.BattleStarted = false
				state.InBattle = false
				state.BattleOver = true
...
func buildWebState(state *GameState, logEntries []string) map[string]interface{} {
	winner := ""
	if state.BattleOver {
		playerCard := &state.Player.Deck[state.PlayerActiveIdx]
		aiCard := &state.AI.Deck[state.AIActiveIdx]
